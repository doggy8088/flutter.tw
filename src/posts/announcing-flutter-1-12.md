...
Golden 影象指的是一個主影象檔案，它是 widget、state、應用或其它您選擇捕捉的視覺內容的正確渲染結果。在 Flutter 1.12 中，我們實現了 [GoldenFileComparator](https://api.flutter.cn/flutter/flutter_test/GoldenFileComparator-class.html) 和 [LocalFileComparator](https://api.flutter.cn/flutter/flutter_test/LocalFileComparator-class.html) 類，它們依照畫素而不是位元來進行比較，因此可以徹底消除錯誤的比較結果。這些新的實現強調呈現視覺差異，從而更清楚地展現出 Golden 影象和正在測試中的更新檔案之間的差異。
