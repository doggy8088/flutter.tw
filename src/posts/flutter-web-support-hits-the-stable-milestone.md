...
當用戶在 web 上導航時，超連結也至關重要。url_launcher package 中的一個新的 [link](https://pub.dev/documentation/url_launcher/latest/link/Link-class.html) widget 使使用者能夠透過深連結直達您應用內的錨點或外部網站。您可以在相關的 widget 上使用 link，包括按鈕、內聯文字、影象，並指定連結是在同一個標籤頁還是新標籤頁中開啟。
